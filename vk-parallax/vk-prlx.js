	...
	function prlx_scr() {
		...
		jQuery('.vk-prlx').each(function(i) {
			...
			if (prlx_start_Y < scrolled_Y) {
				...
				// 開始位置からスクロールした量
				var prlx_scrolled_Y = (scrolled_Y - prlx_start_Y);
				// 背景画像を移動させる距離
				var bg_position_Y = prlx_scrolled_Y * bg_offset_distance;
				...
				if (0 < scroll_rate) {
					...
					bg_position_Y = bg_position_Y - over_offset_H;

					// 背景画像が遅れて上がる場合（背景画像の上方向が見えるようになる）
				} else if (0 > scroll_rate) {
					...
					bg_position_Y = bottom_offset + bg_position_Y;
					// 画像がはみ出している分を更に下方向に補正
					// ※必要高さよりも背景画像が高い場合は下端がはみ出した状態でスタートさせる形になる
					//  → 背景画像の位置を下げるために オフセット量がプラス方向に増える
					bg_position_Y = bg_position_Y + over_offset_H;
