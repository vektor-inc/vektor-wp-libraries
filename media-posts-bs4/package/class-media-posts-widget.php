...
class WP_Widget_Media_Posts_BS4 extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		wp_reset_postdata();
		/* wp_reset_query() でリセットしないと $wp_query がリセットされないので誤動作を引き起こす */
		wp_reset_query();
	...
	function form( $instance ) {
		...
		?>
		<h3 class="admin-custom-h3">
			<label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e( 'Offset count', 'lightning-pro' ); ?></label>
		</h3>
		<input type="text" id="<?php echo $this->get_field_id( 'offset' ); ?>" name="<?php echo $this->get_field_name( 'offset' ); ?>" value="<?php echo $instance['offset']; ?>" class="admin-custom-input" />
	...
	function update( $new_instance, $instance ) {
		...
		$instance['col_xxl']                    = lightning_sanitize_number( $new_instance['col_xxl'] );
