...
if ( ! class_exists( 'VK_Post_Type_Manager' ) ) {
	...
	class VK_Post_Type_Manager {
		...
		public static function add_meta_box_action() {
			 ...
			 * Post Type ID
			 */
			echo '<h4>' . esc_html__( 'Post Type ID(Required)', 'vk_post_type_manager_textdomain' ) . '</h4>';
			echo '<p>' . esc_html__( '20 characters or less in alphanumeric', 'vk_post_type_manager_textdomain' ) . '</p>';
			 ...
			 * Supports(Required)
			 */
			echo '<h4>' . esc_html__( 'Supports(Required)', 'vk_post_type_manager_textdomain' ) . '</h4>';
			 ...
			 * Menu position
			 */
			echo '<h4>' . esc_html__( 'Menu position(optional)', 'vk_post_type_manager_textdomain' ) . '</h4>';
			echo '<p>' . esc_html__( 'Please enter a number.', 'vk_post_type_manager_textdomain' ) . '</p>';
			 ...
			 * Export to Rest api
			 */
			echo '<h4>' . esc_html__( 'Corresponds to the block editor ( optional )', 'vk_post_type_manager_textdomain' ) . '</h4>';
			...
			echo '<label><input type="radio" id="veu_post_type_export_to_api" name="veu_post_type_export_to_api" value="true"' . checked( $export_to_api_value, 'true', false ) . '> ' . esc_html__( 'Corresponds to the block editor ( Export to REST API / optional )', 'vk_post_type_manager_textdomain' ) . '</label>';
			echo '<br />';
			echo '<label><input type="radio" id="veu_post_type_export_to_api" name="veu_post_type_export_to_api" value="false"' . checked( $export_to_api_value, 'false', false ) . '> ' . esc_html__( 'Does not correspond to the block editor', 'vk_post_type_manager_textdomain' ) . '</label>';

			echo '<p>' . esc_html__( 'If you want to use the block editor that, you have to use the REST API.', 'vk_post_type_manager_textdomain' ) . '</p>';
			echo '<hr>';

			/*******************************************
			 * Custom taxonomies
			 */
			echo '<h4>' . esc_html__( 'Custom taxonomies(optional)', 'vk_post_type_manager_textdomain' ) . '</h4>';

			echo '<p>';
			echo esc_html__( 'Custom taxonomy is like a category in post.', 'vk_post_type_manager_textdomain' ) . '<br />';
			echo esc_html__( 'However, it refers to the "category" itself, not to the “item” of the category.', 'vk_post_type_manager_textdomain' ) . '<br />';
			echo esc_html__( 'For example, if you create a post type "construction result", Custom taxonomy will be "construction type", "construction area", etc.', 'vk_post_type_manager_textdomain' );
			echo '</p>';
			...
			for ( $i = 1; $i <= 5; $i++ ) {
				...
				// slug.
				echo '<td>' . esc_html__( 'Custon taxonomy name(slug)', 'vk_post_type_manager_textdomain' ) . '</td>';
				...
				if ( ! in_array( $locale, array( 'en_US', 'en_CA', 'en_NZ', 'en_AU', 'en_ZA', 'en_GB' ) ) ) {
					echo '<div>' . esc_html__( '* Please be sure to enter it with one-byte alphanumeric characters', 'vk_post_type_manager_textdomain' ) . '</div>';
				}
				echo '</td>';

				// 表示名.
				echo '<tr>';
				echo '<td>' . esc_html__( 'Custon taxonomy label', 'vk_post_type_manager_textdomain' ) . '</td>';
				...
				echo '<td>' . esc_html__( 'Hierarchy', 'vk_post_type_manager_textdomain' ) . '</td>';
				echo '<td><label><input type="checkbox" id="veu_taxonomy[' . esc_attr( $i ) . '][tag]" name="veu_taxonomy[' . esc_attr( $i ) . '][tag]" value="true"' . esc_attr( $checked ) . '> ' . esc_html__( 'Make it a tag (do not hierarchize)', 'vk_post_type_manager_textdomain' ) . '</label></td>';
				echo '</tr>';
				...
				echo '<td>' . esc_html__( 'Corresponds to the block editor', 'vk_post_type_manager_textdomain' ) . '</td>';
				echo '<td>';
				echo '<label><input type="radio" id="veu_taxonomy[' . esc_attr( $i ) . '][rest_api]" name="veu_taxonomy[' . esc_attr( $i ) . '][rest_api]" value="true"' . checked( $checked, 'true', false ) . '> ' . esc_html__( 'Corresponds to the block editor ( Export to REST API / optional )', 'vk_post_type_manager_textdomain' ) . '</label>';
				echo '<br />';
				echo '<label><input type="radio" id="veu_taxonomy[' . esc_attr( $i ) . '][rest_api]" name="veu_taxonomy[' . esc_attr( $i ) . '][rest_api]" value="false"' . checked( $checked, 'false', false ) . '> ' . esc_html__( 'Does not correspond to the block editor', 'vk_post_type_manager_textdomain' ) . '</label>';
				echo '</td>';
				echo '</tr>';
